...
- **Trigger name**: `trg_kit_granos_{descriptive_name}`
- **Function name**: `trg_kit_granos_{descriptive_name}_fn`

Where `kit_granos` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_kit_granos_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_kit_granos_audit_log"
AFTER INSERT OR UPDATE ON "module_kit_granos_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_kit_granos_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_kit_granos_*` triggers when the plugin
